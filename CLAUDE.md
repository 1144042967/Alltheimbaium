...
- 右击打开配置 GUI（`ClockMenu`/`ClockScreen`，纯配置界面，无物品栏）：
  - **全局开关**：所有时钟共享，初始值来自配置 `clock.default_active`（默认开），每次进游戏重新加载，不持久化（按钮文本用 ASCII 冒号；英文全局文本 "Global" 以适配按钮宽度）
  - **单独开关**：当前时钟总开关（NBT 持久化）
  - **六方向开关**：逐面控制是否对相邻方块生效，按钮显示该方向实际相邻方块的物品图标（有方块时只显示图标居中、无方块时显示方向名）（NBT 持久化）；有贴图时 hover 显示目标方块名称和方向 tooltip
...
- 六面输出按钮可逐面循环切换 11 个状态：随机 / 禁用 / 槽1~槽9；按钮显示为 [相邻贴图/方向名] ← [槽位贴图/槽号 或 随机/禁用]，有贴图时省略方向名与槽号、内容整体居中（中文用文字，其他语言用符号：方向 ↓↑▲▼◀▶、随机 ?、禁用 ×）；hover 始终显示输出目的/输出方向/输出材料三行说明（无相邻方块显示"无目标"、无槽位贴图显示槽号/随机/禁用）；"输出"按钮控制整体主动输出开关
- 自动向六个面传输物品，支持管道抽取
...
- **右键打开 GUI**（化学品储罐风格，`LiquidFountainMenu`/`LiquidFountainScreen`）：
  ...
  - `- 槽` 只读，玩家/管道可取走
  - 中间背景块上六个按键逐面控制主动输出开关（NBT 持久化），按钮有相邻方块贴图时缩放居中显示贴图（不显示方向文字），无贴图时显示方向名；有贴图时 hover 显示输出目的/输出方向 tooltip
