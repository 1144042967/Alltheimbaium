...
- 可以输入任何单一流体
- 如果输入的液体达到 1 万桶（可配置，默认 10,000,000 mB），将变为无限
- 一个机器可以支持1种流体
- 模型为流体储罐外观，内部液体随存量动态渲染
- 右键打开 GUI：左上 + 槽放入液体桶/带液容器自动输入、空桶/空容器自动装出，处理完的桶/容器移到下方 - 槽；中间六个按键逐面控制主动输出；右上进度条与信息面板展示存量/阈值进度
- 无限后会自动向六个面传输流体（受 GUI 开关控制）
